...
(function () {
  ...
  // 語言選項枚舉
  var Language=/*#__PURE__*/function(Language){Language["en"]="en";Language["zh"]="zh";return Language}(Language||{});/**
   * 用戶配置類，負責管理腳本的各項設置，並註冊菜單。
   */class Config{constructor(){/** 是否開啟偵錯模式 */this.Debug=GM_getValue("Debug",false);/** 結束頁是否自動關閉 */this.IsEndClose=GM_getValue("IsEndClose",true);/** 是否自動加入書櫃 */this.AutoAddBookcase=GM_getValue("AutoAddBookcase",true);/** 自動加入書櫃的封鎖名單（書ID陣列） */this.AutoAddBookcaseBlockade=GM_getValue("AutoAddBookcaseBlockade",[]);/** 是否攔截alert */this.IsHookAlert=GM_getValue("IsHookAlert",true);/** 攔截alert的訊息封鎖名單 */this.HookAlertBlockade=GM_getValue("HookAlertBlockade",[["添加成功"],["刪除成功!"],["恭喜您，该章节已经加入到您的书签！"]]);/** 語言設定 */this.Language=GM_getValue("Language","zh");this.set();this.registerConfigMenu();return this}/**
     * 註冊所有配置項的菜單
  ...
  return Array.from(document.querySelectorAll("a")).find(link=>link.textContent==="下一章")}/**
     * 構造函數，根據當前頁面自動分派對應處理
     */constructor(){/** 常用選擇器集合 */this.SELECTORS={nextPage:["body > div.container > div.mybox > div.page1 > a:nth-child(4)","body > div.mainbox > div > div.page1 > a:nth-child(4)"],authorInfo:"body > div.container > div.mybox > div.txtnav > div.txtinfo.hide720 > span:nth-child(2)",titleDiv:"body > div.container > div.mybox > div.tools",searchInput:"body > header > div > form > div > div.inputbox > input[type=text]",searchForm:"body > header > div > form"};/**
     * 各種頁面判斷與數據獲取方法集合
     */this.data={// 判斷是否有書籍信息
  ...
  IsTwkan:location.host==="twkan.com"};try{if(config.Debug){console.debug(this.debugInfo());}// #tag search
